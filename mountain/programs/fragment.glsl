...
vec3 getSky(vec3 p, vec3 rd) {
    vec3 col = vec3(0.5, 0.8, 1.0);  // Celeste sky color
...
vec3 render(vec2 uv) {
    ...
    if (dist < MAX_DIST) {
        col += getLight(p, rd);
        // Color water, mountains, and sky
        if (getWater(p) < 0.0) {
            col = vec3(0.0, 0.0, 1.0);  // Blue water
        } else if (p.y > 100.0) {
            col = vec3(1.0, 1.0, 1.0);  // Snowy mountains
        } else {
            col = vec3(0.6, 0.4, 0.3);  // Default terrain color
        }
